...
const getMealPlanPrompt = (payload) => {
  ...
  return `
        ...
        **Your Task & Strict Rules:**
        ...
        5.  **NON-NEGOTIABLE:** Do NOT omit any of the four mandatory fields for any meal. Every meal must have a title and a recipe. If a meal (e.g., breakfast) is not requested for a day, do not include a placeholder for it.
        6.  **RESPONSE INTEGRITY:** Ensure the final output is a single, valid JSON object and nothing else. No introductory text, no apologies, no explanations.

        **Output Format:**
        - Respond with ONLY a valid JSON object. Do not include any text before or after the JSON.
        - The root object must contain a "meal_plan" key, which is an array of day objects.
        - If a shopping list is needed, include a "shopping_list" key at the root level.
        - Example structure: \`{ "meal_plan": [ { "day": "Day 1", "breakfast": {...}, "lunch": {...}, "dinner": {...} } ], "shopping_list": [...] }\`
    `;
};
...
app.post("/api/generate-plan", async (req, res) => {
  try {
    ...
    console.log("--- RAW AI RESPONSE for /api/generate-plan ---", responseText); // Log the raw response
